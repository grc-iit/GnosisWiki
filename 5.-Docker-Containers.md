...
![docker-compose exec hostname results](images/5/5.3.5.docker-exec-hostname.png)
...
## 5.3.6. Interactive shell with cluster nodes

First, you should list the docker-compose containers.
```bash
sudo docker-compose ps
```

For me, this lists:
![docker-compose ps result](images/5/5.3.5.docker-ps.png)

Therefore, to interact with node1, we would use the following:
```bash
sudo docker start -i 53containerizedclusters_node1_1
```

## 5.3.7. Shutdown the cluster
```
