...
Here we create two nodes: node1 and node2. The nodes have a shared directory
${HOST_HOME} and pass the environment variable HOST_USER to the Dockerfile.
...
First we have to build the container images for the cluster.
```bash
...
## 5.3.7. Use docker-compose to execute commands
...
To get an interactive shell a node in the cluster, do the following
```bash
